...
const categories = ["System", "Network", "File", "Process", "User", "Package", "Service", "Other"]

const SQL_SCRIPT = `-- 建立 CentOS 指令表
...
-- 建立更新時間觸發器
DROP TRIGGER IF EXISTS update_centos_commands_updated_at ON centos_commands;
CREATE TRIGGER update_centos_commands_updated_at 
    BEFORE UPDATE ON centos_commands 
    FOR EACH ROW 
...
export default function CentOSCommandsPage() {
  const [commands, setCommands] = useState<CentOSCommand[]>([])
  const [searchTerm, setSearchTerm] = useState("")
  const [isAddDialogOpen, setIsAddDialogOpen] = useState(false)
  const [editingCommand, setEditingCommand] = useState<CentOSCommand | null>(null)
  ...
  const checkDatabaseStatus = async () => {
    try {
      ...
      // 嘗試查詢資料表是否存在
      const { data, error } = await supabase.from("centos_commands").select("count", { count: "exact" }).limit(1)
      ...
      if (error && error.code === "42P01") {
        ...
        toast({
          title: "🔧 需要設定資料庫",
          description: "CentOS 指令資料表尚未建立，請按照指示設定",
        })
      } else if (error) {
        throw error
      } else {
  ...
  const loadCommands = async () => {
    try {
      ...
      toast({
        title: "✅ 同步成功",
        description: `已載入 ${data?.length || 0} 個 CentOS 指令`,
      })
    } catch (error) {
      console.error("載入 CentOS 指令失敗:", error)
      toast({
        title: "❌ 載入失敗",
        description: error instanceof Error ? error.message : "無法從雲端載入 CentOS 指令",
        variant: "destructive",
      })
    } finally {
      setIsSyncing(false)
  ...
  // 編輯指令
  const handleEditCommand = (command: CentOSCommand) => {
  ...
  const handleDeleteCommand = async (id: string) => {
    ...
    try {
      const { error } = await supabase.from("centos_commands").delete().eq("id", id)
  ...
  const getCategoryColor = (category: string) => {
    const colors = {
      ...
      Network: "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300",
      File: "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300",
      Process: "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300",
      User: "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300",
      Package: "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300",
      ...
      <div className="min-h-screen bg-gradient-to-br from-red-50 via-white to-orange-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 flex items-center justify-center">
        <Card className="w-96 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm border-0 shadow-2xl">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-4">正在檢查 CentOS 指令資料表...</p>
            <div className="flex items-center justify-center space-x-2">
              <RefreshCw className="w-4 h-4 animate-spin text-red-500" />
              <span className="text-sm text-red-600">請稍候</span>
            </div>
          </CardContent>
        </Card>
      </div>
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400 font-medium">
                    <span>Linux 系統指令參考 • {commands.length} 個指令</span>
              ...
              <CardContent className="py-12">
                <div className="max-w-4xl mx-auto">
                  <div className="text-center mb-8">
                    <div className="w-24 h-24 bg-gradient-to-br from-red-100 to-red-200 dark:from-red-700 dark:to-red-800 rounded-full flex items-center justify-center mx-auto mb-6">
                      <Database className="w-12 h-12 text-red-600 dark:text-red-400" />
                    </div>
                    <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">設定 CentOS 指令資料庫</h3>
                    <p className="text-gray-500 dark:text-gray-400 mb-6">
                      CentOS 指令資料表尚未建立，請按照以下步驟在 Supabase 中執行 SQL 腳本
                    </p>
                  </div>
                      ...
                      <div className="bg-gray-900 dark:bg-gray-800 rounded-lg p-4 mb-4">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-green-400 text-sm font-medium">CentOS Terminal</span>
                          </div>
                          <Button
                            size="sm"
                            variant="ghost"
                ...
                <Card className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm border-0 shadow-xl">
                  <CardContent className="py-16 text-center">
                    ...
                    <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
                      {searchTerm ? "找不到符合條件的指令" : "尚無 CentOS 指令"}
                    </h3>
                    <p className="text-gray-500 dark:text-gray-400 mb-6">
                      {searchTerm ? "請嘗試其他關鍵字" : "開始建立您的第一個 CentOS 指令"}
                    </p>
                    {!searchTerm && isOnline && (
                      <Button
            ...
            <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-4 border border-red-200 dark:border-red-800">
              <div className="flex items-start space-x-3">
                ...
                <div className="text-sm text-red-700 dark:text-red-300">
                  <p className="font-medium mb-2">需要在 Supabase 中執行以下 SQL 腳本來建立 CentOS 指令資料表：</p>
                </div>
              </div>
            </div>
