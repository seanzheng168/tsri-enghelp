...
import { Terminal, ArrowLeft, Plus, Edit, Trash2, Copy, Search, Save, X } from "lucide-react"
...
const defaultCategories = ["Volume", "Aggregate", "Network", "Storage", "Snapshot", "System"]
...
const defaultCommands: Command[] = [
  ...
  {
    id: "3",
    title: "查看 Aggregate 狀態",
    command: "aggr show",
    description: "顯示所有 aggregate 的狀態資訊",
    category: "Aggregate",
  },
  {
    id: "4",
  ...
  },
  {
    ...
    description: "為指定的 volume 建立 snapshot",
    category: "Snapshot",
  },
]
...
export default function NetAppCommandsPage() {
  ...
  // 載入儲存的指令
  useEffect(() => {
    const savedCommands = localStorage.getItem("netapp-commands")
    if (savedCommands) {
      try {
  ...
  const saveCommands = (updatedCommands: Command[]) => {
    ...
    localStorage.setItem("netapp-commands", JSON.stringify(updatedCommands))
  }
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-gray-900 dark:text-white">NetApp 常用指令速查</h1>
                  <p className="text-sm text-gray-500 dark:text-gray-400">NetApp 系統指令參考手冊</p>
                </div>
              </div>
            </div>
